...
def placeholder(size: int = 1024) -> Image.Image:
    ...
    draw.polygon(
        [(28 * unit, 33 * unit), (40 * unit, 16 * unit), (48 * unit, 16 * unit), (34 * unit, 34 * unit)],
        fill="white",
    )
    draw.polygon(
        [(34 * unit, 34 * unit), (48 * unit, 48 * unit), (40 * unit, 48 * unit), (28 * unit, 33 * unit)],
        fill="white",
    )
    return image


def padded(logo: Image.Image, size: int, share: float, background: tuple[int, int, int, int]) -> Image.Image:
...
def main() -> int:
    ...
    parser.add_argument("--placeholder", action="store_true", help="write a placeholder brand/logo.png")
    ...
    if not source.is_file():
        print("brand/logo.png is missing. Add your logo, or run with --placeholder.", file=sys.stderr)
        return 1
    ...
    logo.save(ROOT / "packaging" / "icon.ico", sizes=[(s, s) for s in (16, 24, 32, 48, 64, 128, 256)])
